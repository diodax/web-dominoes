...
@if ($level == 1)
    <svg id="left-level-{{ $level }}" x="-100%" y="25%" width="60" height="120" style="overflow: visible;">
@else
    <svg id="left-level-{{ $level }}" x="-200%" y="0%" width="60" height="120" style="overflow: visible;">
@endif
    ...
    @if (!is_null($child->child))
        @include('game.partials.node-left', ['child' => $child->child , 'level' => $level + 1])
    @endif
</svg>
