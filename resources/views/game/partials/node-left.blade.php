...
@if ($level == 1)
    <svg id="left-level-{{ $level }}" x="{{ $xPosLevel1Attr }}" y="0%" width="60" height="120" style="overflow: visible;">
@else
    <svg id="left-level-{{ $level }}" x="{{ $xPosLevelPlusAttr }}" y="0%" width="60" height="120" style="overflow: visible;">
@endif
    ...
    @if (!is_null($child->child))
        @include('game.partials.node-left', ['child' => $child->child , 'level' => $level + 1, 'isParentDouble' => ($child->head == $child->tail)])
    @endif
</svg>
